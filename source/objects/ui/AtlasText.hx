    ...
    for (splitStr in str.split(''))
    {
      switch (splitStr)
      {
        ...
        case char:
          ...
          {
            charSprite = new AtlasChar(0, 0, atlas, char, fontString);
          }
          else
          {
  ...
  public function getWidth():Int
  {
    ...
    for (char in this.text.split(''))
    {
      switch (char)
      {
        ...
        case char:
          var sprite = new AtlasChar(0, 0, atlas, char, fontString);
          sprite.revive();
          sprite.char = char;
          sprite.alpha = 1;
...
class AtlasChar extends FlxSprite
{
  ...
  public function new(x = 0.0, y = 0.0, atlas:FlxAtlasFrames, char:String, font:String)
  {
    super(x, y);
    _font = font;
    loadImage();
  ...
  function set_char(value:String):String
  {
    ...
    if (animation.curAnim?.name == null) loadImage(); // because Paths.clearUnusedMemory() can kill this atlas
  ...
  function loadImage() frames = Paths.getSparrowAtlas(_font, null, true, "fonts/");
...
// @:nullSafety // trust me, i know what i'm doing here (i don't really know)
private class AtlasFontData
{
  ...
  {
    atlas = Paths.getSparrowAtlas(name, null, true, "fonts/");
    if (atlas == null) {
      trace('Could not find font atlas for font "${name}".');
      return;
    }
