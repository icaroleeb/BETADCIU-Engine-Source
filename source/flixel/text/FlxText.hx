...
class FlxText extends FlxSprite
{
	...
	function regenGraphic():Void
	{
		...
		switch(borderStyle)
		{
			...
			case OUTLINE_FAST | OUTLINE | OUTLINE_FULL:
	...
	function applyBorderStyle():Void
	{
		...
		switch(borderStyle)
		{
			...
			case OUTLINE_FAST | OUTLINE | OUTLINE_FULL if (borderSize < 0):
				_graphicOffset.set(-borderSize, -borderSize);
			
			case NONE | OUTLINE_FAST | OUTLINE | OUTLINE_FULL:
		...
		switch (borderStyle)
		{
			...
			case OUTLINE_FULL:
				// Render an outline around the text
				// (do 8 offset draw calls)
				applyFormats(_formatAdjusted, true); // Apply border color/formatting

				final originX = _matrix.tx;
				final originY = _matrix.ty;
				final realBorderSize = Std.int(borderSize);

				// Loop through a full square around the center
				for (dx in -realBorderSize...realBorderSize + 1) {
					for (dy in -realBorderSize...realBorderSize + 1) {
						if (dx != 0 || dy != 0) {
							_matrix.tx = originX;
							_matrix.ty = originY;
							copyTextWithOffset(dx, dy);
						}
					}
				}

				// Reset the matrix for the main text
				_matrix.tx = originX;
				_matrix.ty = originY;
