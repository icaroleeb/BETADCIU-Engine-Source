...
class BonusSongsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(WeekData.weeksList.length < 1)
		{
			...
			MusicBeatState.switchState(new states.ErrorState("NO BONUS SONGS DETECTED\n\nPress ACCEPT to go to the Week Editor Menu.\nPress BACK to return to Main Menu.\nMake sure that your week files have the "+'"-bonus"'+" suffix to appear here.",
	...
	override function update(elapsed:Float)
	{
		...
		else if (persistentUpdate && controls.ACCEPT && !FlxG.keys.pressed.ALT && !playerBonus.playingMusic)
		{
			persistentUpdate = false;
