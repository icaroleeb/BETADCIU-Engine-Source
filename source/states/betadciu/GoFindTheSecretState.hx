...
class GoFindTheSecretState extends MusicBeatState
{
	...
	var sonicBG2:FlxBackdrop;

	override public function create():Void
	{
		FlxG.sound.playMusic(Paths.music('nexus_bf'));

		sonicBG = new FlxBackdrop(Paths.image('secret/majinBG'), XY, 0, 0);
		add(sonicBG);
		...
		+ '\n of the embed character!' + '\n Go find non-embed character!';
	...
	override function update(elapsed:Float)
	{
		if (FlxG.keys.justPressed.ESCAPE)
		{
			...
			if (PlayState.isBETADCIU)
				if (PlayState.instance.canDoSticker) 
					openSubState(new substates.StickerSubState(null, (sticker) -> new states.betadciu.BETADCIUState(sticker)));
				else 
					MusicBeatState.switchState(new states.betadciu.BETADCIUState());
			else if (PlayState.isBonus)
				if (PlayState.instance.canDoSticker) 
					openSubState(new substates.StickerSubState(null, (sticker) -> new states.betadciu.BonusSongsState(sticker)));
				else 
